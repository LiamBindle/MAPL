...
contains
   ...
   subroutine initialize_io_servers(this, comm, unusable, rc)
     ...
     call splitter%add_group(this%cap_options%npes_model, name='model', isolate_nodes=.true.)
     running_o_server = .false.
     if (this%cap_options%npes_output_server > 0 .or. this%cap_options%nodes_output_server > 0) then
        running_o_server = .true.
        call splitter%add_group(this%cap_options%npes_output_server,nnodes=this%cap_options%nodes_output_server, name='o_server', isolate_nodes=.true.)
     end if
